<script lang="ts">
  ...
  // Оверлей открыт пунктом «✏️ Редактировать» контекстного меню (startEditing):
  // под редактором просмотра не было — «Отмена» и сохранение закрывают оверлей
  // целиком, а не «проваливаются» в просмотр, который юзер не открывал.
  // Редактирование же, начатое кнопкой ✏️ внутри просмотра, отменой возвращает
  // к просмотру. Флаг фиксируется при монтировании (проп осмыслен только там).
  ...
  function cancelEdit(): void {
    if (openedFromMenu) {
      // Пришли из контекстного меню: просмотра под редактором нет — закрываем.
      onClose();
      return;
    }
    editing = false;
