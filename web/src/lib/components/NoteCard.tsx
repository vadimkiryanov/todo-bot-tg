// Карточка заметки: превью первой строки (с форматированием), в левом
// верхнем углу — 📌 у закреплённой (вне потока, текст не сдвигает).
// Приоритет показывается цветной обводкой карточки
...
export function NoteCard({ note, onOpen, onMenu, highlighted = false }: NoteCardProps) {
  ...
  return (
    <button
      type="button"
      className={`glass-card relative flex w-full touch-pan-y select-none items-start gap-2.5 rounded-2xl px-4 py-3 text-left shadow-sm btn-press-soft [-webkit-touch-callout:none]${highlighted ? ' note-highlight' : ''}${prioCls !== '' ? ` ${prioCls}` : ''}`}
      onClick={onCardClick}
    ...
    >
      {/* Закрепление — вне потока: иконка висит в левом верхнем углу
          карточки и не сдвигает превью (иначе у закреплённой текст
          начинался бы правее, чем у остальных). */}
      {note.pinned && (
        <span className="pointer-events-none absolute -right-1 -top-1 text-[11px] leading-none bg-accent rounded-full p-1">📌</span>
      )}
