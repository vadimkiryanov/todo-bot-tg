...
| Метод | Путь | Описание | Тело → Ответ |
|---|---|---|---|
| GET | `/api/v1/topics` | Все топики | → `[{id, name, note_count, pinned}]` |
| POST | `/api/v1/topics` | Создать | `{name}` → 201 `{id, name, note_count, pinned}` |
| PATCH | `/api/v1/topics/{id}` | Переименовать / закрепить | `{name?, pinned?}` → 200 `{id, name, note_count, pinned}` |
...
> `pinned` — закреп топика = быстрый топик бота (`user_quick_topics`, строка кнопок внизу):
> закреп синхронизируется между вебом и ботом. `pinned: true` в PATCH добавляет топик в быстрые
> (`409`, если уже 10), `pinned: false` — убирает; закреп/открепление идемпотентны.
> В PATCH оба поля опциональны, можно передать одно или оба сразу
> (пустое тело `{}` → `400`). В вебе закреплённые показываются первыми в списке табов.
