...
- [ ] Убрать глобальный `sync.Mutex` из `Service`:
...
- [ ] Перестать проглатывать ошибки:
  ...
  - `deleteNoteFiles`: возвращать ошибку, вызывающий решает (`DeleteTopic`/`DeleteNote`)
  - Воркер: ошибки не `continue` молча, а логировать через `slog`
- [ ] Закрыть `GetNoteByID` (чтение без проверки владельца):
