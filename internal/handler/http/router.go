...
func NewRouter(users UserRepository, sessions session.Store, svc TodoService, sessionTTL time.Duration, cookieSecure bool, botToken string) http.Handler {
	...
	mux.Handle("GET /api/v1/notifications", withAuth(todo.listNotifications))
	mux.Handle("POST /api/v1/notifications/read", withAuth(todo.markNotificationsRead))

	mux.Handle("GET /api/v1/folders", withAuth(todo.listFolders))
	mux.Handle("POST /api/v1/folders", withAuth(todo.createFolder))
	mux.Handle("PATCH /api/v1/folders/{id}", withAuth(todo.renameFolder))
	mux.Handle("DELETE /api/v1/folders/{id}", withAuth(todo.deleteFolder))
